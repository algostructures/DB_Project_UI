...
public class OrderDaoImpl implements OrderDao {
        ...
        try{
            ...
            while(rs.next()){
                Orders or = new Orders(rs.getString("who"),rs.getString("id"), rs.getString("vevhile_model"),
                        ...
                        rs.getDouble("tax"), new SimpleDateFormat("yyyy-MM-dd").parse(rs.getString("date")));
                OrderList.add(or);
            }
            return OrderList;
    ...
    public List<Orders> getOrders(Date from, Date to) {
        ...
        try{
            ...
            rs = ps.getResultSet();
            while(rs.next()){
                Date tempDate = new SimpleDateFormat("yyyy-MM-dd").parse(rs.getString("date"));
                if(tempDate.after(from) && tempDate.before(to)) {
                    Orders or = new Orders(rs.getString("who"),rs.getString("id"), rs.getString("vechile_model"),
                            ...
                            rs.getDouble("price"), rs.getDouble("tax"), new SimpleDateFormat("yyyy-MM-dd").parse(rs.getString("date")));
